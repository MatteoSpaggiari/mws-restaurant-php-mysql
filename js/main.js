...
createRestaurantHTML = (restaurant) => {
    ...
    cont_rest.id = `restaurant_${restaurant.id}`;
    li.append(cont_rest);

    const favorite = document.createElement('span');
    favorite.tabIndex = 0;
    favorite.className = 'favorite';
    favorite.setAttribute('data-favorite',String(restaurant.is_favorite));
    if(String(restaurant.is_favorite) == "true") {
        favorite.innerHTML = '&#9733;';
    } else {
        favorite.innerHTML = '&#9734;';
    }
    cont_rest.append(favorite);

    favorite.addEventListener('click', function(event) {
        DBHelper.putIsFavoriteRestaurantDatabase(restaurant.id,favorite.getAttribute('data-favorite'),favorite);
    });

    favorite.addEventListener('focus', function(event) {
        window.addEventListener("keydown", function(event) {
            const key = event.charCode || event.keyCode;
            if(key === 13 || key === 32) //Only when the key pressed is tab
            {
